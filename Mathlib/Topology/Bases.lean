...
* `IsTopologicalBasis s`: The topological space `t` has basis `s`.
...
theorem isTopologicalBasis_of_subbasis {s : Set (Set α)} (hs : t = generateFrom s) :
  ...
  refine' ⟨_, _, le_antisymm (le_generateFrom _) <| generateFrom_mono fun t ht => _⟩
...
theorem isTopologicalBasis_infᵢ {β : Type _} {ι : Type _} {X : ι → Type _}
  ...
  have := (isTopologicalBasis_pi cond).inducing (inducing_infᵢ_to_pi _)
