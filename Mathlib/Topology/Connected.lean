...
theorem IsPreconnected.subset_clopen (hs : IsPreconnected s) (hu : IsClopen u)
    (hsu : (s ∩ u).Nonempty) : s ⊆ u :=
  hs.subset_left_of_subset_union hu.isOpen hu.compl.isOpen disjoint_compl_right (by simp) hsu
...
theorem connectedSpace_iff_connectedComponent :
  ...
  · rintro ⟨⟨x⟩⟩
    exact
      ⟨x, eq_univ_of_univ_subset <| is_preconnected_univ.subset_connectedComponent (mem_univ x)⟩
  · rintro ⟨x, h⟩
...
theorem preconnectedSpace_iff_connectedComponent :
  ...
  constructor
  · intro h x
    exact eq_univ_of_univ_subset <| is_preconnected_univ.subset_connectedComponent (mem_univ x)
  · intro h
...
instance (priority := 100) IrreducibleSpace.connectedSpace (α : Type u) [TopologicalSpace α]
    [IrreducibleSpace α] : ConnectedSpace α where toNonempty := IrreducibleSpace.toNonempty α
...
theorem isPreconnected_iff_preconnectedSpace {s : Set α} : IsPreconnected s ↔ PreconnectedSpace s :=
  ⟨Subtype.preconnectedSpace, by
    intro
    simpa using is_preconnected_univ.image ((↑) : s → α) continuous_subtype_val.continuous_on⟩
...
theorem connectedComponent_subset_interᵢ_clopen {x : α} :
  ...
  subset_Inter fun Z => Z.2.1.connectedComponent_subset Z.2.2
...
theorem QuotientMap.preimage_connectedComponent [TopologicalSpace β] {f : α → β}
  ...
  ((preimage_connectedComponent_connected h_fibers (fun _ => hf.is_closed_preimage.symm)
    ...
    (hf.continuous.maps_to_connectedComponent a)
...
theorem connectedComponentIn_mem_nhds [LocallyConnectedSpace α] {F : Set α} {x : α} (h : F ∈ 𝓝 x) :
  ...
  exact mem_nhds_iff.mpr ⟨s, h2s.is_preconnected.subset_connectedComponentIn hxs hsF, h1s, hxs⟩
...
theorem isOpen_connectedComponent [LocallyConnectedSpace α] {x : α} :
  ...
  exact is_open_univ.connectedComponentIn
  ...
  · intro h
    ...
    refine' fun x U hU =>
        ⟨connectedComponentIn (interior U) x,
          ...
          mem_connectedComponentIn _, is_connected_connectedComponentIn_iff.mpr _⟩ <;>
...
theorem Continuous.connectedComponentsLift_continuous (h : Continuous f) :
  ...
  h.quotient_lift_on' h.image_eq_of_connectedComponent_eq
...
theorem Continuous.connectedComponentsLift_unique (h : Continuous f) (g : ConnectedComponents α → β)
  ...
  connectedComponents_lift_unique' <| hg.trans h.connectedComponents_lift_comp_coe.symm
...
instance ConnectedComponents.totallyDisconnectedSpace :
  ...
  rw [← connectedComponents.quotient_map_coe.image_connectedComponent, ←
