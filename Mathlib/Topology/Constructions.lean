...
@[simp 900] lemma continuous_to_bool [TopologicalSpace α] {f : α → Bool} :
    Continuous f ↔ IsClopen (f ⁻¹' {true}) := by
  rw [continuous_to_discrete, Bool.forall_bool, IsClopen, ← isOpen_compl_iff, ← preimage_compl,
    Bool.compl_singleton_true, and_comm]
...
theorem continuous_isLeft : Continuous (isLeft : α ⊕ β → Bool) :=
  continuous_to_bool.2 ⟨by simpa only [range_inl] using isOpen_range_inl,
