...
class HasTop (α : Type u) where
  /-- The top element. Use notation `⊤` (`\top`) for this. -/
  top : α
...
class HasBot (α : Type u) where
  /-- The bottom element. Use notation `⊥` (`\bot`) for this. -/
  bot : α
#align has_bot HasBot

/-- The top term in its type. -/
notation "⊤" => HasTop.top

/-- The bottom term in its type. -/
notation "⊥" => HasBot.bot
...
@[ext] -- why not? should probably not port this until we
class OrderTop (α : Type u) [LE α] extends HasTop α where
  /-- Every `a : α` is at most `⊤ : α` -/
  le_top : ∀ a : α, a ≤ ⊤
...
noncomputable def topOrderOrNoTopOrder (α : Type _) [LE α] : PSum (OrderTop α) (NoTopOrder α) := by
  ...
  · push_neg  at H
    -- porting note: in Lean 3 this next line was just
    -- exact psum.inl ⟨_, classical.some_spec H⟩ }
    exact PSum.inl (@OrderTop.mk α _ { top := Classical.choose H} (Classical.choose_spec H))
...
theorem OrderTop.ext_top {α} {hA : PartialOrder α} (A : OrderTop α) -- must use hA
    {hB : PartialOrder α} -- now we have to be careful of diamonds
    (B : @OrderTop α hB.toPreorder.toLE) -- we ensure B uses hB
    -- If the two ≤ s are the same
    (H : ∀ x y : α, hA.toPreorder.toLE.le x y ↔ hB.toPreorder.toLE.le x y) :
    -- then the two corresponding tops must be the same
    (@toHasTop α hA.toPreorder.toLE A).top = (@toHasTop α hB.toPreorder.toLE B).top :=
-- porting note: `apply le_top` worked after the `rw` in Lean 3
top_unique <| by rw [← H]; exact @le_top _ hA.toPreorder.toLE A _

#align order_top.ext_top OrderTop.ext_top
--set_option pp.all true

theorem OrderTop.ext' {α} [h : PartialOrder α] {A B : OrderTop α} : A = B :=
by
  apply OrderTop.ext
  apply OrderTop.ext_top
  intros
  rfl
...
@[ext] -- why not; this wasn't in mathlib3 though
class OrderBot (α : Type u) [LE α] extends HasBot α where
  /-- `⊥ : α` is at most every `a : α` -/
  bot_le : ∀ a : α, ⊥ ≤ a
...
noncomputable def botOrderOrNoBotOrder (α : Type _) [LE α] : PSum (OrderBot α) (NoBotOrder α) := by
  ...
  · push_neg at H
    exact PSum.inl (@OrderBot.mk α _ { bot := Classical.choose H} (Classical.choose_spec H))
...
instance hasTop [HasBot α] : HasTop αᵒᵈ :=
  ⟨(⊥ : α)⟩

instance hasBot [_root_.HasTop α] : HasBot αᵒᵈ :=
  ⟨(⊤ : α)⟩

instance orderTop [LE α] [OrderBot α] : OrderTop αᵒᵈ :=
  { _root_.OrderDual.hasTop α with le_top := @bot_le α _ _ }

instance orderBot [LE α] [OrderTop α] : OrderBot αᵒᵈ :=
...
theorem OrderBot.ext_bot {α} {hA : PartialOrder α} (A : OrderBot α) -- must use hA
    {hB : PartialOrder α} -- now we have to be careful of diamonds
    (B : @OrderBot α hB.toPreorder.toLE) -- we ensure B uses hB
    -- If the two ≤ s are the same
    (H : ∀ x y : α, hA.toPreorder.toLE.le x y ↔ hB.toPreorder.toLE.le x y) :
    -- then the two corresponding tops must be the same
    (@toHasBot α hA.toPreorder.toLE A).bot = (@toHasBot α hB.toPreorder.toLE B).bot :=
-- porting note: `apply le_top` worked after the `rw` in Lean 3
bot_unique <| by rw [← H]; exact @bot_le _ hA.toPreorder.toLE A _

theorem OrderBot.ext' {α} [PartialOrder α] {A B : OrderBot α} : A = B :=
OrderBot.ext _ _ $ OrderBot.ext_bot _ _ $ λ _ _ => Iff.rfl
...
-- this is an arbitary #exit. We've got to get to `Disjoine` on line 2230
#exit
