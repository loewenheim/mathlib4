...
structure Config : Type where
  ...
  attrs : Array Syntax := #[]
  /-- Set to true when the `to_additive` call is nested inside another `to_additive` call.
  This happens when translating a lemma about `pow` to `smul` to `vadd`.
  This disables attribute warnings -/
  isNested := false
...
def warnExt [Inhabited σ] (stx : Syntax) (ext : PersistentEnvExtension α β σ) (f : σ → Name → Bool)
  ...
  if f (ext.getState (← getEnv)) src then
    Linter.logLint linter.existingAttributeWarning stx
      m!"The source declaration {src} has attribute {attrName}.
...
warnExt stx attr.ext (f ·.stateStack.head!.state ·) thisAttr attrName src
...
def warnParametricAttr (stx : Syntax) (attr : ParametricAttribute β)
  (thisAttr attrName src : Name) : CoreM Unit :=
warnExt stx attr.ext (·.contains ·) thisAttr attrName src
...
def applyAttributes (stx : Syntax) (attrs : Array Syntax) (thisAttr src tgt : Name) : TermElabM Unit := do
  ...
  warnAttr stx simpExtension (·.lemmaNames.contains <| .decl ·) thisAttr `simp src
  warnAttr stx normCastExt.up (·.lemmaNames.contains <| .decl ·) thisAttr `norm_cast src
  warnAttr stx normCastExt.down (·.lemmaNames.contains <| .decl ·) thisAttr `norm_cast src
  warnAttr stx normCastExt.squash (·.lemmaNames.contains <| .decl ·) thisAttr `norm_cast src
  warnAttr stx pushCastExt (·.lemmaNames.contains <| .decl ·) thisAttr `norm_cast src
  warnAttr stx Std.Tactic.Ext.extExtension (fun b n => (b.elements.any fun t => t.declName = n))
    thisAttr `ext src
  warnAttr stx Mathlib.Tactic.reflExt (·.elements.contains ·) thisAttr `refl src
  warnAttr stx Mathlib.Tactic.symmExt (·.elements.contains ·) thisAttr `symm src
  warnAttr stx Mathlib.Tactic.transExt (·.elements.contains ·) thisAttr `trans src
  warnAttr stx Std.Tactic.Coe.coeExt (·.contains ·) thisAttr `coe src
  warnParametricAttr stx Lean.Linter.deprecatedAttr thisAttr `deprecated src
  warnParametricAttr stx Std.Tactic.Lint.nolintAttr thisAttr `nolint src
  -- add attributes
  -- the following is similar to `Term.ApplyAttributesCore`, but we hijack the implementation of
  -- `simp`, `simps` and `thisAttr`.
...
-/
def copyMetaData (cfg : Config) (src tgt : Name) : CoreM Unit := do
  ...
  MetaM.run' <| Elab.Term.TermElabM.run' <| applyAttributes cfg.ref cfg.attrs `to_additive src tgt
...
`to_additive` calls. -/
def addToAdditiveAttr (src : Name) (cfg : Config) : AttrM (Array Name) :=
