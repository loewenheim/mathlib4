...
-- Porting note: `include` doesn't exist anymore
-- include F
...
-/
-- Porting note: `has_nonempty_instance` is not a Mathlib4 linter (yet?)
@[nolint unusedArguments]
def InducedCategory (_ : C → D) : Type u₁ := C
...
-/
-- Porting note: should have `nolint has_nonempty_instance` attribute as well
@[ext]
