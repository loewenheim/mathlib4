...
theorem eq_nil (v : Vector3 α 0) : v = ([] : Vector3 α 0) :=
...
@[simp]
theorem append_left
    (i : Fin2 m) (v : Vector3 α m) (w : Vector3 α n) : (v +-+ w) (left n i) = v i := by
  induction i <;> cases v using Vector3.consElim <;> simp_all[left]
#align vector3.append_left Vector3.append_left

@[simp]
theorem append_add
    (v : Vector3 α m) (w : Vector3 α n) (i : Fin2 n) : (v +-+ w) (add i m) = w i := by
  induction m with
  | zero => rfl
  | succ m ih =>
    cases v using Vector3.consElim
    simp[ih, add]
...
#align vectorEx_iff_exists vectorEx_iff_exists
...
#align vectorAll_iff_forall vectorAll_iff_forall
...
theorem vectorAllp_singleton (p : α → Prop) (x : α) : VectorAllp p [x] = p x :=
  rfl
#align vectorAllp_singleton vectorAllp_singleton

@[simp]
theorem vectorAllp_nil (p : α → Prop) : VectorAllp p [] = True :=
  rfl
#align vectorAllp_nil vectorAllp_nil
...
#align vectorAllp_cons vectorAllp_cons
...
theorem vectorAllp_iff_forall (p : α → Prop) (v : Vector3 α n) : VectorAllp p v ↔ ∀ i, p (v i) :=
  ...
  · simp
    ...
    · have hs := h (fs i)
      simp at hs
      exact hs
#align vectorAllp_iff_forall vectorAllp_iff_forall
